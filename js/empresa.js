...
const toast=t=>{
  ...
  setTimeout(()=>x.classList.add("hidden"),2600);
};
...
const safeId=s=>String(s||"")
  ...
  .replace(/[^a-z0-9]+/g,"-")
  .replace(/(^-|-$)/g,"")
  .slice(0,30)+"-"+Math.floor(Math.random()*999);
...
function eventResponse(nonce){
  ...
  return company?.eventResponses?.[String(nonce)] || null;
}

function confirmedDecisionHtml(){
  const label=company?.lastDecisionLabel || "Decisão registrada";
  return `
    <div class="notification">
      <strong>✅ Decisão confirmada</strong><br>
      ${label}<br>
      <span class="muted">Aguarde o professor avançar para a próxima rodada.</span>
    </div>
  `;
}
...
function closeEventModal(){
  ...
  if($("#eventoOpcoes")) $("#eventoOpcoes").innerHTML="";
}
...
async function saveWholeRoom(nextRoom){
  const f=await getFirebase();
  if(f){
    await f.set(f.ref(f.db,`rooms/${roomCode}`),nextRoom);
  }else{
...
async function sendNegotiation(payload){
  ...
  if(f){
    await f.set(f.ref(f.db,`rooms/${roomCode}/negotiations/${id}`),data);
  }else{
    room=demoGet(`room:${roomCode}`,room);
    room.negotiations=room.negotiations||{};
    room.negotiations[id]=data;
    demoSet(`room:${roomCode}`,room);
  }
...
async function updateNegotiation(id,changes){
  const latest=await getRoom();
  if(!latest?.negotiations?.[id]) return false;

  const n=latest.negotiations[id];
  Object.assign(n,changes);
  n.updatedAt=Date.now();

  latest.negotiations[id]=n;
...
function shouldReceiveCurrentEvent(currentEvent){
  ...
  const joinedAt=Number(company?.joinedAt||0);
  const nonce=Number(currentEvent.nonce||0);

  if(company?.ignoreEventsBefore && nonce<=Number(company.ignoreEventsBefore)){
    return false;
  }

  if(joinedAt && nonce<joinedAt){
    return false;
  }
...
function onRoomChange(){
  ...
  if(!currentEvent?.nonce){
    ...
    const nonce=String(currentEvent.nonce);
    const answered=eventResponse(nonce);

    if(!shouldReceiveCurrentEvent(currentEvent)){
      lastEventNonce=currentEvent.nonce;
      closeEventModal();
    }else if(answered){
      lastEventNonce=currentEvent.nonce;
      closeEventModal();
    }else if(currentEvent.nonce!==lastEventNonce){
      lastEventNonce=currentEvent.nonce;
      showEvent(currentEvent.id,currentEvent.nonce);
    }
  }
...
function investmentHtml(){
  ...
  const saved=company.investmentPlan||{
    estrutura:0,pessoas:0,marketing:0,tecnologia:0,estoque:0,reserva:0
  };

  return `
    <div class="stack">
      <p class="muted">
        Distribua recursos entre as seis áreas. O total não pode ultrapassar o caixa disponível.
      ...
      <button class="primary" id="confirmarPlanoInvest">
...
function bindInvestmentPlan(){
  ...
  const updateSummary=()=>{
    ...
    if(box){
      box.textContent=
        `Total alocado: ADM$ ${total.toLocaleString("pt-BR")} • `+
        `Saída real do caixa: ADM$ ${gasto.toLocaleString("pt-BR")} • `+
        `Saldo projetado: ADM$ ${saldo.toLocaleString("pt-BR")}`;
    }
  };
  ...
  btn.onclick=async()=>{
    ...
    company.lastDecisionLabel="💰 Plano de investimentos confirmado";

    await saveCompany();
    notify("💰 Plano de investimentos confirmado.");
...
async function submitAuctionBid(amount){
  ...
  const payload={companyId,companyName:company.name,amount:value,createdAt:Date.now()};
  const f=await getFirebase();

  if(f){
    await f.set(f.ref(f.db,`rooms/${roomCode}/auction/bids/${companyId}`),payload);
  }else{
    room=demoGet(`room:${roomCode}`,room);
  ...
  if(a.status==="open"){
    return `
      <div class="stack">
        ...
        <input id="valorLance" type="number"
          min="${Number(a.minBid||0)}"
        ...
        <p class="muted">🔒 Apenas o professor verá os valores dos lances.</p>
  ...
  if(a.status==="closed"){
    if(a.winnerId===companyId){
      return `
        <div class="notification">
          ...
          O prêmio foi colocado no Inventário Estratégico.
        </div>
      `;
    }
    ...
    return `
      <div class="notification">
        ...
        ${a.winnerName?`Vencedora: <strong>${a.winnerName}</strong> — ADM$ ${Number(a.winningBid||0).toLocaleString("pt-BR")}`:"Não houve lance vencedor."}
      </div>
    `;
  }
...
function ensureInventoryButtons(){
  ...
  const campanhaLine=$("#campanha")?.parentElement;
  if(campanhaLine && !$("#usarCampanha")){
    const b=document.createElement("button");
    b.id="usarCampanha";
    b.textContent="ATIVAR CAMPANHA VIRAL";
    b.style.marginLeft="10px";
    campanhaLine.appendChild(b);
    b.onclick=activateViralCampaign;
  }
}

async function activateViralCampaign(){
  if(Number(company.campanha||0)<=0){
    toast("Sua empresa não possui Campanha Viral.");
    return;
  }

  if(company.campaignActive){
    toast("A Campanha Viral já está ativa.");
    return;
  }

  company.clientes=Number(company.clientes||0)+15;
  company.reputacao=Number(company.reputacao||0)+10;
  company.xp=Number(company.xp||0)+10;
  company.campaignActive=true;
  company.campaignActivatedAt=Date.now();
  company.campaignBonus={clientes:15,reputacao:10,xp:10};

  await saveCompany();
  notify("📣 Campanha Viral ativada: +15 clientes, +10 reputação e +10 XP.");
  toast("Campanha Viral ativada!");
}
...
function decisionHtml(r){
  if(company?.lastDecisionRound===r && [1,2,4,7,8].includes(r)){
    return confirmedDecisionHtml();
  }
  if(r===1){
    return `<div class="stack">
      <button data-d="crescimento">🚀 Foco em crescimento</button>
      <button data-d="equilibrio">⚖️ Gestão equilibrada</button>
      <button data-d="seguranca">🛡️ Segurança financeira</button>
    </div>`;
  }
  ...
  if(r===4){
    return `<div class="stack">
      <button data-d="preco">💲 Competir por preço</button>
      <button data-d="qualidade">💎 Competir por qualidade</button>
      <button data-d="pessoas">👥 Investir em pessoas</button>
      <button data-d="inovacao">💡 Inovar</button>
    </div>`;
  }
  if(r===7){
    return `<p><b>⚡ BATALHA:</b> Uma empresa pode aumentar as vendas e, ao mesmo tempo, piorar o caixa?</p>
      <div class="stack"><button data-d="quiz-sim">SIM</button><button data-d="quiz-nao">NÃO</button></div>`;
  }
  if(r===8){
    return `<p><b>👾 BOSS FINAL:</b> Custos +15%, vendas -20%. Escolham a reação principal.</p>
      <div class="stack">
        <button data-d="boss-reserva">🛡️ Usar reserva e renegociar</button>
        <button data-d="boss-corte">✂️ Cortar investimentos</button>
        <button data-d="boss-credito">🏦 Tomar crédito</button>
      </div>`;
  }
...
function bindDecision(){
  document.querySelectorAll("[data-d]").forEach(b=>{
    b.onclick=async()=>{
      ...
      document.querySelectorAll("[data-d]").forEach(x=>x.disabled=true);
      ...
      company.lastDecisionRound=room.round;
      company.lastDecisionCode=d;
      company.lastDecisionLabel=DECISION_LABELS[d]||d;
      company.lastDecisionAt=Date.now();

      await saveCompany();
      notify(`✅ Decisão confirmada: ${DECISION_LABELS[d]||d}`);
...
function showEvent(id,nonce){
  ...
  const key=String(nonce||room?.currentEvent?.nonce||"");
  if(!key||!shouldReceiveCurrentEvent(room?.currentEvent)){
    closeEventModal();
    return;
  }
  ...
  document.querySelectorAll("[data-o]").forEach(b=>{
    b.onclick=async()=>{
      if(eventResponse(key)){
        closeEventModal();
        toast("Este evento já foi respondido.");
        return;
      }

      document.querySelectorAll("[data-o]").forEach(x=>x.disabled=true);
      const optionIndex=Number(b.dataset.o);
      const option=ev.options[optionIndex];
      if(!option){closeEventModal();return;}

      Object.entries(option.delta).forEach(([k,v])=>{
        company[k]=(company[k]||0)+v;
      });

      company.eventResponses=company.eventResponses||{};
      ...
      await saveCompany();
      closeEventModal();
      notify(`✅ Evento resolvido: ${ev.title} — ${option.label}`);
...
function ensureCompanyAccessFields(){
  ...
  wrap.innerHTML=`
    ...
    <input id="senhaEmpresa" type="password" autocomplete="current-password"
      placeholder="Crie ou digite a senha da empresa" minlength="4" maxlength="30">
    <p class="muted" style="margin:0">
      Primeiro acesso: crie uma senha. Nos próximos acessos, use a mesma sala, o mesmo nome da empresa e esta senha.
    </p>
  `;
...
function ensureNegotiationFields(){
  ...
  const enviar=$("#enviarProposta");
  if(!destino||!proposta||!enviar) return;
  ...
  box.innerHTML=`
    ...
    <select id="negTipo">
      <option value="mensagem">Acordo / parceria</option>
      <option value="venda-campanha">Vender Campanha Viral</option>
    </select>
    <div id="negVendaCampos" class="hidden">
      <label>Valor pedido (ADM$)</label>
      <input id="negValor" type="number" min="1" step="1000" placeholder="Ex.: 8000">
    </div>
    <div id="negHistorico" class="notifications"></div>
  `;
  proposta.parentNode.insertBefore(box,proposta);
  $("#negTipo")?.addEventListener("change",()=>{
    $("#negVendaCampos")?.classList.toggle("hidden",$("#negTipo").value!=="venda-campanha");
  });
}
...
function rememberAccess(){
  try{
    localStorage.setItem("adm360:lastCompanyAccess",JSON.stringify({
      roomCode,companyId,name:company?.name||""
    }));
  }catch{}
}
...
async function enterCompany(){
  ...
  if(existing){
    ...
    company={
      ...
      eventResponses:{},
      joinedAt:now,
      ignoreEventsBefore:Number(room?.currentEvent?.nonce||now)
    };

    await saveCompany();
    toast(`Empresa ${company.name} criada. Eventos anteriores não serão aplicados.`);
  }
...
function negotiationCard(id,n){
  const incoming=normalizeName(n.to)===normalizeName(company.name);
  const outgoing=normalizeName(n.from)===normalizeName(company.name);
  ...
  const status=statusLabels[n.status]||n.status||"PENDENTE";

  let actions="";
  if(incoming && n.status==="pending"){
    actions=`
      <div class="stack" style="margin-top:8px">
        <button data-neg-action="accept" data-neg-id="${id}">✅ ACEITAR</button>
        <button data-neg-action="refuse" data-neg-id="${id}">❌ RECUSAR</button>
        <button data-neg-action="counter" data-neg-id="${id}">↩️ CONTRAPROPOSTA</button>
      </div>
    `;
  }
  ...
  return `
    <div class="notification">
      <strong>${incoming?"Recebida de":"Enviada para"} ${incoming?n.from:n.to}</strong><br>
      ${n.type==="venda-campanha"
        ? `📣 Campanha Viral — ADM$ ${Number(n.value||0).toLocaleString("pt-BR")}`
        : n.message||"Proposta"}
      <br><small>Status: ${status}</small>
      ${n.counterMessage?`<br><small>Contraproposta: ${n.counterMessage}</small>`:""}
      ${actions}
    </div>
  `;
}
...
function renderNegotiations(){
  ensureNegotiationFields();
  const box=$("#negHistorico");
  if(!box||!company||!room) return;

  const entries=Object.entries(room.negotiations||{})
    .filter(([id,n])=>
      normalizeName(n.to)===normalizeName(company.name) ||
      normalizeName(n.from)===normalizeName(company.name)
    )
    .sort((a,b)=>Number(b[1].createdAt||0)-Number(a[1].createdAt||0));

  box.innerHTML=entries.slice(0,6).map(([id,n])=>negotiationCard(id,n)).join("")
    || `<p class="muted">Nenhuma negociação registrada.</p>`;

  document.querySelectorAll("[data-neg-action]").forEach(btn=>{
    btn.onclick=()=>handleNegotiationAction(btn.dataset.negId,btn.dataset.negAction);
  });

  const signature=entries.map(([id,n])=>`${id}:${n.status}:${n.updatedAt||n.createdAt}`).join("|");
  if(signature!==lastNegotiationSignature){
    lastNegotiationSignature=signature;
    const incoming=entries.find(([id,n])=>normalizeName(n.to)===normalizeName(company.name));
    if(incoming){
      const [id,n]=incoming;
      if(n.status==="pending") notify(`🤝 Nova proposta de ${n.from}.`);
      if(n.status==="accepted") notify(`✅ Negociação com ${n.from} foi concluída.`);
      if(n.status==="refused") notify(`❌ Proposta de ${n.from} recusada.`);
      if(n.status==="countered") notify(`↩️ ${n.from} enviou contraproposta.`);
    }
  }
}

async function completeCampaignSale(negotiationId,n){
  const latest=await getRoom();
  const seller=findCompanyInRoom(latest,n.from);
  const buyer=findCompanyInRoom(latest,n.to);

  if(!seller||!buyer){
    toast("Empresa vendedora ou compradora não encontrada.");
    return false;
  }
  ...
  if(Number(buyer.caixa||0)<value){
    toast("A empresa compradora não possui caixa suficiente.");
    return false;
  }
  if(Number(seller.campanha||0)<=0){
    toast("A empresa vendedora não possui Campanha Viral.");
    return false;
  }

  buyer.caixa=Number(buyer.caixa||0)-value;
  seller.caixa=Number(seller.caixa||0)+value;

  seller.campanha=Number(seller.campanha||0)-1;
  buyer.campanha=Number(buyer.campanha||0)+1;

  if(seller.campaignActive){
    seller.clientes=Math.max(0,Number(seller.clientes||0)-15);
    seller.reputacao=Math.max(0,Number(seller.reputacao||0)-10);
    seller.campaignActive=false;
    seller.campaignSoldAfterActivation=true;
    seller.campaignSoldAt=Date.now();
  }

  buyer.campaignActive=false;
  buyer.campaignReceivedAt=Date.now();

  latest.companies[seller.id]=seller;
  latest.companies[buyer.id]=buyer;

  const neg=latest.negotiations[negotiationId];
  neg.status="accepted";
  neg.completedAt=Date.now();
  neg.history=neg.history||[];
  neg.history.push({
    action:"accepted-and-transferred",
    by:buyer.name,
    at:Date.now(),
    value
  });

  latest.negotiations[negotiationId]=neg;
  await saveWholeRoom(latest);
  return true;
}

function findCompanyInRoom(r,name){
  const target=normalizeName(name);
  return Object.values(r?.companies||{}).find(c=>normalizeName(c?.name)===target)||null;
}

async function handleNegotiationAction(id,action){
  const n=room?.negotiations?.[id];
  if(!n) return;

  if(action==="refuse"){
    await updateNegotiation(id,{
      status:"refused",
      respondedBy:company.name,
  ...
  if(action==="counter"){
    const text=prompt("Digite a contraproposta:");
    if(!text) return;

    const latest=await getRoom();
    const original=latest?.negotiations?.[id];
    if(!original) return;
    ...
    await updateNegotiation(id,{
      status:"countered",
      counterMessage:text,
      counteredBy:company.name
    });
    await sendNegotiation(counter);
    toast("Contraproposta enviada.");
    return;
  }

  if(action==="accept"){
    if(n.type==="venda-campanha"){
      const ok=await completeCampaignSale(id,n);
      if(ok) toast("✅ Compra concluída e Campanha Viral transferida.");
      return;
    }
...
$("#enviarProposta")?.addEventListener("click",async()=>{
  ...
  const type=$("#negTipo")?.value||"mensagem";

  if(!to){
    toast("Informe a empresa destinatária.");
    return;
  }

  const target=findCompanyByName(to);
  if(!target){
    toast("Empresa destinatária não encontrada nesta sala.");
    return;
  }
  if(target.id===companyId){
    toast("Não é possível negociar com a própria empresa.");
    return;
  }

  if(type==="venda-campanha"){
    const value=Math.floor(Number($("#negValor")?.value||0));
    if(Number(company.campanha||0)<=0){
      toast("Sua empresa não possui Campanha Viral para vender.");
      return;
    }
    if(!value||value<=0){
      toast("Informe o valor da Campanha Viral.");
      return;
    }
    ...
    await sendNegotiation({
      ...
      resource:"campanha",
      value,
      message:message||`Venda de Campanha Viral por ADM$ ${value.toLocaleString("pt-BR")}`
    });

    toast("Proposta de venda enviada.");
  }else{
    if(!message){
      toast("Digite a proposta.");
      return;
    }
  ...
  $("#proposta").value="";
  if($("#negValor")) $("#negValor").value="";
});

console.log("ADM Arena 360 — Empresa carregada com eventos, inventário e negociação completa.");
