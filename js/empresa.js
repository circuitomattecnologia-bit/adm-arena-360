...
async function sendNegotiation(payload){
  ...
  if(f){
    await f.set(f.ref(f.db,`rooms/${roomCode}/negotiations/${id}`),payload);
  }else{
    room=demoGet(`room:${roomCode}`,room);
...
function onRoomChange(){
  ...
  incoming.slice(-1).forEach(([id,n])=>{
    notify(`🤝 Proposta de ${n.from}: ${n.message}`);
  });
}
...
function bindInvestmentPlan(){
  ...
  btn.onclick=async()=>{
    ...
    notify(
      `💰 Plano de investimentos confirmado. Estrutura ${v.estrutura.toLocaleString("pt-BR")}, `+
      `Pessoas ${v.pessoas.toLocaleString("pt-BR")}, `+
      `Marketing ${v.marketing.toLocaleString("pt-BR")}, `+
      `Tecnologia ${v.tecnologia.toLocaleString("pt-BR")}, `+
      `Estoque ${v.estoque.toLocaleString("pt-BR")}, `+
      `Reserva ${v.reserva.toLocaleString("pt-BR")}.`
    );
...
async function submitAuctionBid(amount){
  ...
  notify(
    `🔨 Lance de ADM$ ${value.toLocaleString("pt-BR")} registrado. `+
    `As outras empresas não veem o seu valor.`
  );
}
...
function auctionHtml(){
  ...
  if(!a){
    return `<p class="muted">Aguardem o professor abrir o leilão.</p>`;
  }
    ...
    return `
      <div class="stack">
        <div class="notification">
          ...
          <strong>Seu caixa atual:</strong>
          ADM$ ${Number(company.caixa||0).toLocaleString("pt-BR")}
        </div>
        ...
        <p class="muted">
          🔒 Apenas o professor verá os valores dos lances.
          Você pode atualizar o seu lance enquanto o leilão estiver aberto.
        </p>

        ${
          myBid
            ? `<p><strong>✅ Lance registrado:</strong> ADM$ ${Number(myBid.amount||0).toLocaleString("pt-BR")}</p>`
            : ""
        }
      </div>
    `;
    ...
    if(a.winnerId===companyId){
      return `
        <div class="notification">
          ...
          ${a.title||""}<br>
          Valor pago:
          <strong>ADM$ ${Number(a.winningBid||0).toLocaleString("pt-BR")}</strong><br>
...
function decisionHtml(r){
  ...
  if(r===7){
    return `
      <p>
        <b>⚡ BATALHA:</b>
        Uma empresa pode aumentar as vendas e, ao mesmo tempo, piorar o caixa?
      </p>
      <div class="stack">
        <button data-d="quiz-sim">SIM</button>
        <button data-d="quiz-nao">NÃO</button>
      </div>
    `;
  }
  ...
  if(r===8){
    return `
      <p>
        <b>👾 BOSS FINAL:</b>
        Custos +15%, vendas -20%. Escolham a reação principal.
      </p>
      <div class="stack">
  ...
  return `
    <p class="muted">
      Esta rodada depende da interação do professor, eventos ou negociações.
    </p>
  `;
}
...
function bindDecision(){
  document.querySelectorAll("[data-d]").forEach(b=>{
    b.onclick=async()=>{
      ...
      if(d==="seguranca"){
        company.escudo=(company.escudo||0)+1;
        company.xp+=8;
      }
      ...
      if(d==="quiz-sim"){
        company.xp+=10;
        notify(
          "✅ Correto: vendas não significam necessariamente melhora do caixa."
        );
      }

      if(d==="quiz-nao"){
        company.xp=Math.max(0,company.xp-2);
        notify(
          "❌ Revejam: faturamento, custos e fluxo de caixa são diferentes."
        );
      }
...
function showEvent(id,nonce){
  ...
  $("#eventoOpcoes").innerHTML=ev.options
    .map((o,i)=>`<button data-o="${i}">${o.label}</button>`)
    .join("");
...
function findExistingCompany(name){
  ...
  return Object.values(room?.companies||{})
    .find(c=>normalizeName(c?.name)===target)||null;
}
...
function ensureCompanyAccessFields(){
  ...
  wrap.innerHTML=`
    ...
    <p class="muted" style="margin:0">
      Primeiro acesso: crie uma senha. Nos próximos acessos,
      use a mesma sala, o mesmo nome da empresa e esta senha.
    </p>
  `;
...
console.log("ADM Arena 360 — Empresa carregada com controle de eventos e decisões.");
